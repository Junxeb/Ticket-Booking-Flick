...
const AdminSidebar = () => {
    ...
    // เพิ่ม icon ในส่วนของ sidebar ด้านข้าง
    const adminNavLinks = [
        ...
        md:max-w-60 w-full border-r border-gray-300/20 text-sm">
            ...
            <div className="w-full">
                {adminNavLinks.map((link, index) =>(
                    ...
                    text-gray-400 ${isActive && 'bg-red/15 text-red group'}`}>
                    {({ isActive }) => (
                        <>
                        {/* เมื่อคลิกเมนูตรง sidebar จะขึ้นพื้นหลังแดง*/}
                            <link.icon className="w-5 h-5" />
                            <p className="max-md:hidden">{link.name}</p>
                            <span className={`w-1.5 h-10 rounded-l right-0 absolute ${isActive && 'bg-red'}`} />
                        </>
                    )}
                    </NavLink>
                ))}
