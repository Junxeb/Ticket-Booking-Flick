...
// ดึงข้อมูลจาก assets มาใช้ในการทำ movies card เพื่อแสดงในหน้า Home pages
const MovieCard = ({movie}) => {

    const navigate = useNavigate();
    return (
        
        <div className="flex flex-col justify-between p-3 bg-gray-700 
        rounded-2xl hover:-translate-y-1 transition duration-300 w-66">
            ...
            <div className="flex items-center justify-between mt-4 pb-3">
                ...
                className="px-4 py-2 text-xs bg-softred hover:bg-red transition 
                rounded-full font-medium cursor-pointer">Buy Tickets</button>

                <p className="flex items-center gap-1 text-sm text-gray-400 mt-1 pr-1">
                    <StarIcon className="w-4 h-4 text-softred fill-softred"/>
